...
import { DeliveryRequest } from "@daml.js/apple-supply-chain-prototype-0.0.1/lib/Delivery";
...
export default function CBoardingRequest() {
    ...
    function requestDelivery(boardingCid:ContractId<Boarding>)
    {
      boardings.contracts.map(a=>{
        if(a.contractId===boardingCid)
        {
          let withContent={
            content:{
              driver: a.payload.content.driver,
              companyOperator: getDriver(a.payload.content.destination),
              boarding:boardingCid,
              deliveryDate: (new Date()).toISOString()
            }
          }
          ledger.create(DeliveryRequest,withContent);
        }
      });
    }

    function getDriver(companyCid:ContractId<Company>){
      let driver:typeof party="";
      companies.contracts.map(a=>{
        if(a.contractId===companyCid)
        {
          driver=a.payload.operator;
        }
      })
      return driver;
    }
            ...
            {boardings.contracts.map(a => (
              <TableRow key={a.contractId} className={classes.tableRow}>
                ...
                <TableCell key={7} className={classes.tableCellButton}>
                  <Button color="primary" size="small" className={classes.choiceButton} variant="contained" disabled={a.payload.content.driver!==party} 
                  onClick={() => requestDelivery(a.contractId)}>
                    Request Delivery
                  </Button>
                </TableCell>
              </TableRow>
